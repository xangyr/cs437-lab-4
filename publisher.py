...
clientId = "ProcessEmission_Publisher"
thingName = "ProcessEmission_Publisher"
...
while retryCount != 0:
    try:
        ...
        print('DISCORVEY INFO!!! ###', discoveryInfo)
...
idx_arr = []
for i in range(5):
    df = pd.read_csv('./data2/vehicle{}.csv'.format(str(i)))
    df = df['vehicle_CO2']
    df_arr.append(df)
    idx_arr.append(0)
...
# num_list = [0,1]
while num_list:
    ranV = random.choice(num_list)
    message = {}
    message['car'] = ranV
    message['emission'] = df_arr[ranV][idx_arr[ranV]]
    if idx_arr[ranV] == len(df_arr[ranV]) - 1:
        num_list.remove(ranV)
    else:
        idx_arr[ranV] += 1
